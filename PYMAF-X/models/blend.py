...
class Blend(nn.Module):
    ...
    def forward(self, input_batch={}, J_regressor=None, rw_cam={}):
        ...
        # mano_global_orient
        # torch.Size([2, 1, 3, 3])
        pred_orient_lh =data1['mano_global_orient'][0]
        pred_orient_rh =data1['mano_global_orient'][1]
        data2 = joblib.load(file2)
        pred_pose =torch.cat((data2["smpl_global_orient"],data2["smpl_body_pose"]),dim=1)
        batch_size =pred_pose.shape[0]  

        pred_cam_rh = data1["pred_cam"]# torch.Size([2, 3])"
        pred_cam = torch.cat([pred_cam_rh[:, 0:1] * 10., pred_cam_rh[:, 1:] / 10.], dim=1) 
        pred_shape = data2["smpl_betas"]
        ...
        smplx_kwargs = {}
            # if self.full_body_mode:
        pred_lhand_rotmat =data1['mano_hand_pose'][0]
        pred_rhand_rotmat =data1['mano_hand_pose'][1]
        smplx_kwargs['left_hand_pose'] = pred_lhand_rotmat
        smplx_kwargs['right_hand_pose'] = pred_rhand_rotmat
        ...
        out_put.update({
                        ...
                        'pred_rhand_rotmat': pred_rhand_rotmat,})
        return out_put
                #     pred_cam.append(output['theta'][:, :3]) 1
                # pred_verts.append(output['verts']) 1
                # pred_smplx_verts.append(output['smplx_verts'])
                # pred_pose.append(output['theta'][:, 13:85]) 1
                # pred_betas.append(output['theta'][:, 3:13]) 1
                # pred_joints3d.append(output['kp_3d']) 1

                # smplx_params.append({'shape': output['pred_shape'], 1
                ...
                #                     })
def blend(smpl_mean_params, device=torch.device('cuda'), is_train=True):
    """ Constructs an PyMAF model with ResNet50 backbone.
    Args:
        is_train (bool): If True, load ImageNet/COCO pre-trained models
    """
    model = Blend(smpl_mean_params, device, is_train)
    return model