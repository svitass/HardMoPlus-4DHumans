...
class Regressor(nn.Module):
    def __init__(self, feat_dim, smpl_mean_params, use_cam_feats=False, feat_dim_hand=0, feat_dim_face=0, bhf_names=['body'], smpl_models={}):
        ...
        if not self.smpl_mode:
            lhand_mean_rot6d = rotmat_to_rot6d(batch_rodrigues(self.smpl.model.model_neutral.left_hand_mean.view(-1, 3)).view([-1, 3, 3]))
    ...
    def forward(self, x=None, n_iter=1, J_regressor=None, rw_cam={}, init_mode=False, global_iter=-1, **kwargs):
                ...
                if 'body' in self.bhf_names:
                    ...
                    if self.use_cam_feats:
                        if cfg.MODEL.USE_IWP_CAM:
                            ...
                            crop_center = rw_cam['bbox_center'] / torch.cat([rw_cam['img_w'][:, None], rw_cam['img_h'][:, None]], 1)
                    ...
                    if (global_iter == (cfg.MODEL.PyMAF.N_ITER - 1)) and (self.full_body_mode or self.body_hand_mode):
                        ...
                        if cfg.MODEL.PyMAF.OPT_WRIST:
                            ...
                            if cfg.MODEL.PyMAF.ADAPT_INTEGR:
                                ...
                                pred_rotmat_body = torch.cat([pred_rotmat_body[:, :20], 
        ...
        if self.smplx_mode:
            if cfg.MODEL.PyMAF.PRED_VIS_H and global_iter == (cfg.MODEL.PyMAF.N_ITER - 1):
                pred_lhand_filtered = [pred_lhand[_i] if pred_vis_lhand[_i] else self.init_rhand[0] for _i in range(batch_size)]
                pred_rhand_filtered = [pred_rhand[_i] if pred_vis_rhand[_i] else self.init_rhand[0] for _i in range(batch_size)]
                pred_lhand_filtered = torch.stack(pred_lhand_filtered)
                pred_rhand_filtered = torch.stack(pred_rhand_filtered)
                pred_hf6d = torch.cat([pred_lhand_filtered, pred_rhand_filtered, pred_face], dim=1).reshape(batch_size, -1, 6)
            else:
        ...
        if self.hand_only_mode:
            ...
            pred_joints_full = torch.cat([pred_joints, pred_output.lhand_joints, pred_output.rhand_joints, pred_output.face_joints,
                                          pred_output.lfoot_joints, pred_output.rfoot_joints], dim=1)
        else:
            pred_joints_full = pred_joints
        pred_keypoints_2d = projection(pred_joints_full, {**rw_cam, 'cam_sxy': pred_cam}, iwp_mode=cfg.MODEL.USE_IWP_CAM)
        if cfg.MODEL.USE_IWP_CAM:
        ...
        if self.smpl_mode or self.smplx_mode:
            ...
            if self.smplx_mode:
                ...
                output.update({
                               ...
                               'pred_face_rotmat': pred_face_rotmat,
                               'pred_lhand_kp3d': pred_output.lhand_joints,
                               'pred_rhand_kp3d': pred_output.rhand_joints,
                               'pred_face_kp3d': pred_output.face_joints,
                               'pred_lhand_kp2d'  : pred_keypoints_2d[:, len_b_kp:len_b_kp+len_h_kp],
                               'pred_rhand_kp2d'  : pred_keypoints_2d[:, len_b_kp+len_h_kp:len_b_kp+len_h_kp*2],
                               'pred_face_kp2d'  : pred_keypoints_2d[:, len_b_kp+len_h_kp*2:len_b_kp+len_h_kp*2+len_f_kp],
                               'pred_feet_kp2d'  : pred_keypoints_2d[:, len_b_kp+len_h_kp*2+len_f_kp:len_b_kp+len_h_kp*2+len_f_kp+len_feet_kp],
                               })
                if cfg.MODEL.PyMAF.OPT_WRIST:
                    output.update({
...
class PyMAF(nn.Module):
    ...
    def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS, device=torch.device('cuda'), is_train=True):
        ...
        assert cfg.TRAIN.BHF_MODE in ['body_only', 'hand_only', 'face_only', 'body_hand', 'full_body']#bhf for body hand face
        ...
        if 'hand' in self.bhf_names or 'face' in self.bhf_names:
            ...
            self.smpl2lhand = torch.from_numpy(smpl2limb_vert_faces['lhand']['vids']).long()#vids for vertex ID
        ...
        # grid points for grid feature extraction 网格点 for 网格特征提取
        grid_size = 21
        ...
        self.register_buffer('grid_points', grid_points)# 用于在模型中注册一个缓冲区（buffer）。
        # 缓冲区是一种特殊的张量，它在模型中保存状态，但不会被优化器更新，即不会在训练过程中调整其值
        grid_feat_dim = grid_size * grid_size * cfg.MODEL.PyMAF.MLP_DIM[-1]

        # the fusion of grid and mesh-aligned features 融合 of ……
        ...
        if self.fuse_grid_align:
            self.att_feat_reduce = get_fusion_modules(bhf_names, bhf_ma_feat_dim, grid_feat_dim, n_iter=n_iter_att, out_feat_len=bhf_att_feat_dim)
            for part in bhf_names:
        ...
        for i in range(cfg.MODEL.PyMAF.N_ITER):
            ...
            if self.smpl_mode:#这里有好多个regressor，对应每层金字塔缩小
                self.regressor.append(Regressor(feat_dim=ref_infeat_dim, smpl_mean_params=smpl_mean_params, use_cam_feats=cfg.MODEL.PyMAF.USE_CAM_FEAT, smpl_models=self.smpl_family))
            else:
                if cfg.MODEL.PyMAF.MAF_ON:
                    if 'hand' in self.bhf_names or 'face' in self.bhf_names:
                        if i == 0:
        ...
        for part in bhf_names:
            ...
            for i in range(cfg.MODEL.PyMAF.N_ITER):
                ...
                if cfg.MODEL.PyMAF.GRID_ALIGN.USE_ATT and i >= self.att_starts:
                    self.maf_extractor[part].append(MAF_Extractor(filter_channels=filter_channels_default[att_feat_dim_idx:], iwp_cam_mode=cfg.MODEL.USE_IWP_CAM))
                else:
                    self.maf_extractor[part].append(MAF_Extractor(filter_channels=filter_channels, iwp_cam_mode=cfg.MODEL.USE_IWP_CAM))
    ...
    def init_mesh(self, batch_size, J_regressor=None, rw_cam={}):
        ...
        if self.init_mesh_output is None or self.batch_size != batch_size:
            self.init_mesh_output = self.regressor[0](torch.zeros(batch_size), J_regressor=J_regressor, rw_cam=rw_cam, init_mode=True)
                ...
                for hf_i, part_name in enumerate(self.part_names):
                    ...
                    if cfg.MODEL.PyMAF.MAF_ON:
                        ...
                        if limb_rf_i == 0 or cfg.MODEL.PyMAF.GRID_FEAT:
                            limb_ref_feat_ctd = self.maf_extractor[hf_key][limb_rf_i].sampling(grid_points, im_feat=limb_feat_i, reduce_dim=limb_reduce_dim)
                        else:
                        ...
                        if self.fuse_grid_align and limb_rf_i >= self.att_starts:
                            ...
                            limb_grid_feature_ctd = self.maf_extractor[hf_key][limb_rf_i].sampling(grid_points, im_feat=limb_feat_i, reduce_dim=limb_reduce_dim)
                            limb_grid_ref_feat_ctd = torch.cat([limb_grid_feature_ctd, limb_ref_feat_ctd], dim=-1).permute(0, 2, 1)

                            if cfg.MODEL.PyMAF.GRID_ALIGN.USE_ATT:
                                att_ref_feat_ctd = self.align_attention[hf_key][limb_rf_i-self.att_starts](limb_grid_ref_feat_ctd)[0]
                            elif cfg.MODEL.PyMAF.GRID_ALIGN.USE_FC:
                                att_ref_feat_ctd = limb_grid_ref_feat_ctd

                            att_ref_feat_ctd = self.maf_extractor[hf_key][limb_rf_i].reduce_dim(att_ref_feat_ctd.permute(0, 2, 1)).view(batch_size, -1)
            ...
            if 'body' in self.bhf_names:
                if cfg.MODEL.PyMAF.MAF_ON:
                    ...
                    if rf_i == 0 or cfg.MODEL.PyMAF.GRID_FEAT:
                        ref_feature = self.maf_extractor['body'][rf_i].sampling(grid_points, im_feat=s_feat_i, reduce_dim=reduce_dim)
                    else:
                        ...
                        ref_feature = self.maf_extractor['body'][rf_i](pred_smpl_verts_ds, im_feat=s_feat_i, cam={**rw_cam, 'cam_sxy': pred_cam}, add_att=True, reduce_dim=reduce_dim) # [B, 431 * n_feat]

                    if self.fuse_grid_align and rf_i >= self.att_starts:
                        if rf_i > 0 and not cfg.MODEL.PyMAF.GRID_FEAT:
                            grid_feature = self.maf_extractor['body'][rf_i].sampling(grid_points, im_feat=s_feat_i, reduce_dim=reduce_dim)
