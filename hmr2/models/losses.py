...
class Keypoint2DLoss_cliff(nn.Module):
    ...
    def __init__(self, loss_type: str = 'l1'):
        ...
        Args:
            loss_type (str): Choose between l1 and l2 losses.
        """
        super(Keypoint2DLoss_cliff, self).__init__()
        if loss_type == 'l1':
            self.loss_fn = nn.L1Loss(reduction='none')
        elif loss_type == 'l2':
            self.loss_fn = nn.MSELoss(reduction='none')
    ...
    def forward(self, pred_keypoints_2d: torch.Tensor, gt_keypoints_2d: torch.Tensor, loss_keypoints_scale:torch.Tensor) -> torch.Tensor:
        """
        Compute 2D reprojection loss on the keypoints.
        Args:
            pred_keypoints_2d (torch.Tensor): Tensor of shape [B, S, N, 2] containing projected 2D keypoints (B: batch_size, S: num_samples, N: num_keypoints)
        ...
        loss_keypoints = loss * loss_keypoints_scale
        loss_keypoints = loss_keypoints.sum(dim=(1,2))
        return loss.sum()
