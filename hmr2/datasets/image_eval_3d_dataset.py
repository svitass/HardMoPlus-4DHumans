...
from .image_annot_dataset import ImageDataset_annot
...
class ImageDataset_eval(Dataset):
    def __init__(self,
        ...
        """
        super(ImageDataset_eval, self).__init__()
        self.train = train
        self.cfg = cfg
        ...
        if type(img_dir) == list:
            self.dance_img_dir = img_dir[0]
            self.kungfu_img_dir = img_dir[1]
        ...
        self.center = self.data['center'].reshape(len(self.imgname), -1)
        ###我们数据集除了两百
        ...
        try:
            self.pred_keypoints_2d = self.data['pred_keypoints_2d'].astype(np.float32)
        except KeyError:
            print("不存在pred的")
        try: 
        ...
        try:
            body_keypoints_3d = self.data['body_keypoints_3d'].astype(np.float32)[:,:,:3]
        except KeyError:
            body_keypoints_3d = np.zeros((len(self.center), 25, 3), dtype=np.float32)
        # Try to get extra 3d keypoints, if available
        try:
            extra_keypoints_3d = self.data['extra_keypoints_3d'].astype(np.float32)[:,:,:3]
        except KeyError:
            extra_keypoints_3d = np.zeros((len(self.center), 19, 3), dtype=np.float32)
        ...
        conf = np.ones((self.keypoints_3d.shape[0], self.keypoints_3d.shape[1], 1))
        self.keypoints_3d = np.concatenate((self.keypoints_3d, conf), axis=-1)
        # self.pred_keypoints_3d = np.zeros((self.keypoints_3d.shape[0], self.keypoints_3d.shape[1], 3))
        # print(self.keypoints_3d.shape)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if type(self.img_dir) == list:
            base_dance_style = image_file_rel.split('/')[0]
            if base_dance_style in dance_styles:
                image_file = os.path.join(self.dance_img_dir, image_file_rel)
            else:
                image_file = os.path.join(self.kungfu_img_dir, image_file_rel)
        else:
        ...
        # pred_keypoints_2d = self.pred_keypoints_2d[idx].copy()
        # pred_keypoints_3d = self.pred_keypoints_3d[idx].copy()
        ...
        center_x = center[0]
        center_y = center[1]
        ###这个地方我们恢scale参数,训练hmr时候不除以2.0,这个之前一直是/1.2的哈
        ...
        # item['pred_keypoints_2d'] = pred_keypoints_2d.astype(np.float32)
        # item['pred_keypoints_3d'] = pred_keypoints_3d.astype(np.float32)
    ...
    def load_tars_as_webdataset(cfg: CfgNode, urls: str|List[str], train: bool,
                ...
                try:
                    ...
                    for data in datas:
                        if 'detection.npz' in item:
                            ...
                            mask = np.ones_like(item['png'][:,:,0], dtype=bool)
                        yield {
                            '__key__': item['__key__'],
                            'jpg': item['png'],
                            'data.pyd': data,
        ...
        # Process the dataset
        #dataset = dataset.map(lambda x:pyd_plus_list(x))
        ...
        dataset = dataset.map(lambda x: ImageDataset_annot.process_webdataset_tar_item(x, train,
