...
from smplx.lbs import vertices2joints, blend_shapes, vertices2landmarks
    ...
    for part in ['lhand', 'rhand', 'face', 'arm', 'forearm', 'larm', 'rarm', 'lwrist', 'rwrist']:
        ...
        if os.path.exists(part_vid_fname):
            ...
            if part in ['lhand', 'rhand']:
                ...
                smpl2mano_vids = np.array(smpl2mano_id).astype(np.int64)
                mano_faces = get_mano_faces(hand_type='right' if part == 'rhand' else 'left').astype(np.int64)
                ...
                smpl2head_vids = np.unique(body_model_faces[part_body_fid]).astype(np.int64)
                ...
                head_faces = mesh_vid_raw[head_faces].astype(np.int64)
                ...
                smpl2part_vids = np.unique(body_model_faces[part_body_fid]).astype(np.int64)
                ...
                part_faces = mesh_vid_raw[part_faces].astype(np.int64)
