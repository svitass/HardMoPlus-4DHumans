...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # Load dataset config
    #dataset_cfg = dataset_config(name='datasets_half.yaml')
    dataset_cfg = dataset_config(name='datasets_freeman.yaml')
    #print(f"dataset_cfg:{dataset_cfg}\n")
    #print(f"cfg:{cfg}\n")
    # Save configs
    save_configs(cfg, dataset_cfg, cfg.paths.output_dir)

    # Setup training and validation datasets,2d�~B�被�~R�~@�~L~V�~F
    datamodule =HMR2DataModule(cfg, dataset_cfg)
    checkpoint_path = "/data/boning_zhang/.cache/4DHumans/logs/train/multiruns/hmr2/0/checkpoints/epoch=35-step=1000000.ckpt"
    model = HMR2.load_from_checkpoint(checkpoint_path, strict=False, cfg=cfg)
    # Setup model
    #model = HMR2(cfg)
