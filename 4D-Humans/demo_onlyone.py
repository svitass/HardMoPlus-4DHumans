...
def main():
    ...
    parser.add_argument('--notonlyone',dest='notonlyone', action='store_true', default=False, help='')
    ...
    for img_path in sorted(Path(args.img_folder).glob('*.[jp][pn]g')):
        ...
        for batch in dataloader:
            ...
            # Render the result
            batch_size = batch['img'].shape[0] #<=8
            for n in range(batch_size):
                ...
                if args.top_view:
                    top_img = renderer(out['pred_vertices'][n].detach().cpu().numpy(),
                                            out['pred_pred_cam'][n].detach().cpu().numpy(),
                                            white_img,
                                            mesh_base_color=LIGHT_BLUE,
                                            scene_bg_color=(1, 1, 1),
                                            top_view=True)
                ...
                for key in out.keys():
                    if isinstance(out[key], torch.Tensor):   
                        if key in existing_data.keys():
                            existing_data[key] = torch.cat((existing_data[key], out[key]), dim=0)    
                        else:
                            existing_data[key]=out[key]
                    elif isinstance(out[key], dict):
                        son_dict = out[key]
                        for sub_key in son_dict.keys():
                            new_key = 'smpl_' + sub_key
